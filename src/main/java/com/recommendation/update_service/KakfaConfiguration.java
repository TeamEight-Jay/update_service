...
public class KakfaConfiguration {
    ...
    @Bean
    public ConsumerFactory<String, UpdateMessage> updateConsumerFactory() {
        Map<String, Object> config = new HashMap<>();

        config.put(ConsumerConfig.BOOTSTRAP_SERVERS_CONFIG, "127.0.0.1:9092");
        config.put(ConsumerConfig.KEY_DESERIALIZER_CLASS_CONFIG, StringDeserializer.class);
        config.put(ConsumerConfig.VALUE_DESERIALIZER_CLASS_CONFIG, StringDeserializer.class);
        config.put(ConsumerConfig.GROUP_ID_CONFIG, "group_update");
    ...
    @Bean
    public ConcurrentKafkaListenerContainerFactory<String, UpdateMessage> updateKafkaListenerFactory() {
